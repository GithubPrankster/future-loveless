...
func _process(_delta) -> void:
	if Info.opening_out and !Info.it_goes and !casey_anim.is_playing() and !treble_anim.is_playing():
		if Input.is_action_just_pressed("left") and $treble.modulate.a != 1.0:
			...
			Info.chosen_player = Info.PLAYER_TREBLE
		if Input.is_action_just_pressed("right") and $casey.modulate.a != 1.0:
			...
			Info.chosen_player = Info.PLAYER_CASEY
